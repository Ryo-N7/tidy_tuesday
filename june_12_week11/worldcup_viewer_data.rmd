...
worldcup_confed %>% 
  mutate(players = players,
         ...
         players_share = players_per_pop_1000 / sum(players_per_pop_1000)) %>% 
  ggplot(aes(x = players_share, y = tv_audience_share)) +
  geom_point(aes(size = players_per_pop_1000)) +
  geom_label(aes(label = confederation), nudge_x = 0.05, nudge_y = 0.05)
